...
const getOneClient = async (id) => {
  try {
    const result = await client.query("SELECT * FROM Client WHERE id=? ;", [id])
    return result[0]
  } catch (err) {
...
const insertClient = async (newClient) => {
  try {
    ...
    await client.query('INSERT INTO Client(name) VALUES (?);', values)
...
const deleteClient = async (id) => {
  try {
    const values = [id]
    await client.query('DELETE FROM Client WHERE ID=?;', values)
